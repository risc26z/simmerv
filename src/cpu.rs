...
impl Cpu {
    ...
    pub fn new(terminal: Box<dyn Terminal>) -> Self {
        ...
        cpu.x_[10] = 0; // boot hart
        cpu.x_[11] = 0x1020; // start of DTB (XXX could put that elsewhere);
        cpu
    }
    ...
    #[must_use]
    pub fn read_register(&mut self, reg: u8) -> i64 {
    ...
    fn run_cpu_tick(&mut self) {
        ...
        if self.seqno >= 1973743554 {
            ...
            println!(
                "** fetching instruction {:016x} {word:08x} at seqno {}",
                self.insn_addr, self.seqno
            );
        }
        ...
        if self.seqno > 1973743000 {
            ...
            let _ = write!(buf1, "@{}: (use {:?}) ", self.seqno, self.read);
            let _ = write!(buf2, "{buf1:35}");
            self.read = vec![];
            self.disassemble_insn(&mut buf2, self.insn_addr, self.insn);
            println!("{buf2}");
        }
    }
...
fn dump_format_b(s: &mut String, cpu: &mut Cpu, word: u32, address: u64, evaluate: bool) -> usize {
    ...
    *s += get_register_name(f.rs1);
    if evaluate && f.rs1 != 0 {
        let _ = write!(s, ":{:x}", cpu.x_[f.rs1]);
    }
    let _ = write!(s, ", {}", get_register_name(f.rs2));
    if evaluate && f.rs2 != 0 {
        let _ = write!(s, ":{:x}", cpu.x_[f.rs2]);
    }
...
mod test_cpu {
    ...
    fn read_register() {
        ...
        for i in 1..31 {
            cpu.x_[i] = (0xffffffffffffffff - i) as i64;
        }
